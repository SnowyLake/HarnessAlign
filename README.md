...
| 本地导入或来源不明的 Skills | 同步完整文件, 二进制资产与来源记录 |
| GitHub 下载的 Skills | 只同步已安装清单和来源记录, 在接收设备补齐内容 |
...
GitHub 下载的 Skills 不再上传文件内容. 同步后优先复用本机内容哈希匹配的 Skill, 缺失或版本不同的内容从来源仓库下载并校验, 不触发上游更新. 新安装的 Skill 记录固定 commit; 旧来源记录没有 commit 时按原分支下载, 但只有内容哈希匹配才接受, 不会静默替换为最新版. 如果旧分支已变化, 需要在原设备检查并更新 Skill 后重新同步, 或将保留的旧内容作为本地 Skill 导入.

下载或校验失败时保留现有工作区, 报告待重试的 Skill, 下次 Refresh preview 重试尚未完成的同步. 下载完成并通过完整配置校验后才替换本地源文件. GitHub Skill 的本地文件视为可重建缓存, 不同步其中的修改, 也不提示转为本地 Skill; 内容不匹配时按来源重新下载并替换. 旧版完整快照仍可读取, 后续成功同步会从远端当前版本移除可重新下载的 Skill 文件; Git 历史提交中的文件仍然保留.

单个同步文件最多 8 MiB, 配置源总大小最多 32 MiB, 文件与目录总数最多 5000. 这些安全限制仍适用于本地完整备份和下载后恢复的内容. 超限, GitHub 返回不完整目录树, 路径逃逸, symlink 或 junction 都会阻止同步.

...
Generate 和 Setup 使用当前 Layer 顺序与选项. 未保存的其他源文件草稿需先保存才参与生成.
...
`layers` 可以为空, 表示当前项目未选择任何 Layer. `.harness-align/layers/<layer>/` 目录构成 Layer 目录, 并允许暂时不包含 Option. 空 Layer 不能加入 Setup; 未写入 `config.json` 的 Layer 目录仍然有效, 只是不参与生成.

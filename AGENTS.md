...
- 当前配套配置目录是 `C:\Users\fengh\OneDrive\Configs\AI\HarnessAlign`, 其中保留 `.halign`, `.agent-sessions`, `generate.cmd` 和 `setup.cmd`.
- `generate.cmd` 与 `setup.cmd` 属于配套配置目录, 负责切换到自身目录后调用全局 `halign` 命令, 暂停终端并传递退出码.
- 只实现 Codex, Cursor 和 OpenCode 已明确需要的能力. 不为未经需求证明的 Harness, 插件, 模板或扩展点预留抽象.

## 目录职责

- `src/halign.ts` 是 CLI 和生成器的唯一生产实现. 当前规模下保持单文件, 不拆分 package 或创建单实现抽象.
- `tests/halign.test.ts` 是唯一测试源文件, 使用 Node 内置 `node:test` 覆盖解析, 渲染, 生成, 检查, 部署和路径安全.
- `dist/` 是 `tsc` 生成的 JavaScript 输出, 包含 `dist/src/halign.js` 和 `dist/tests/halign.test.js`. 不得手工编辑.
...
- `package.json` 的 `bin.halign` 必须指向 `dist/src/halign.js`. 修改入口路径后必须重新执行 `npm link` 并从外部配置目录验证.
...
6. 从一个真实或隔离的外部 `.halign` 配置目录运行 `halign generate` 和 `halign check`.
7. 修改部署逻辑时使用临时 `USERPROFILE` 验证 `halign setup`, 不触碰真实用户配置.
...
- 工具工程不包含 `.halign`, 因此不得在本仓库根目录使用 npm script 包装 `generate`, `check` 或 `setup`.

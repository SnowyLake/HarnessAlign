...
- 本仓库实现独立的 `hcs` TypeScript CLI, 不保存用户的 `.hcs` 配置源.
- CLI 必须以调用者的当前工作目录作为配置根目录. 不得把工具安装目录或源码目录当作配置根目录.
- 当前配套配置目录是 `C:\Users\fengh\OneDrive\Configs\AI\HarnessConfigSynchro`, 其中保留 `.hcs`, `.agent-sessions`, `generate.cmd` 和 `setup.cmd`.
- `generate.cmd` 与 `setup.cmd` 属于配套配置目录, 负责切换到自身目录后调用全局 `hcs` 命令, 暂停终端并传递退出码.
- 只实现 Codex, Cursor 和 OpenCode 已明确需要的能力. 不为未经需求证明的 Harness, 插件, 模板或扩展点预留抽象.

## 目录职责

- `src/hcs.ts` 是 CLI 和生成器的唯一生产实现. 当前规模下保持单文件, 不拆分 package 或创建单实现抽象.
- `tests/hcs.test.ts` 是唯一测试源文件, 使用 Node 内置 `node:test` 覆盖解析, 渲染, 生成, 检查, 部署和路径安全.
- `dist/` 是 `tsc` 生成的 JavaScript 输出, 包含 `dist/src/hcs.js` 和 `dist/tests/hcs.test.js`. 不得手工编辑.
- `node_modules/` 保存本地依赖, 由 npm 根据 `package-lock.json` 管理. 不得手工编辑或提交其内部文件.
- `package.json` 定义 ESM package, Node 版本, `hcs` 可执行入口, scripts 和直接依赖.
...
- `AGENTS.md` 是本仓库的开发约束, 不属于 HCS 生成结果.

## 命令契约

- `hcs generate [--profile <profile>]` 验证配置并更新当前目录下的 `.hcs/generated/`, 成功时输出受管理文件数量.
- `hcs check [--profile <profile>]` 比较期望输出与 `.hcs/generated/`, 一致时输出成功摘要并返回 `0`, 存在差异时列出差异并返回 `1`.
- `hcs setup [--profile <profile>]` 先生成, 再把结果部署到当前用户已经存在的 Harness 根目录, 成功时输出部署摘要.
- 无效命令或参数输出 usage 并返回 `2`. 领域错误输出到 stderr 并返回 `1`.
- `package.json` 的 `bin.hcs` 必须指向 `dist/src/hcs.js`. 修改入口路径后必须重新执行 `npm link` 并从外部配置目录验证.
...
- 输入必须先完整验证, 再修改 `.hcs/generated/` 或用户部署目录.
...
- `.hcs/config.json` 定义版本, 输出标题, Profile 和 Harness 列表.
- `.hcs/rules/` 中的根 Rule 按 `(priority, repository_relative_path)` 排序, 再按 Harness targets 独立过滤.
- `.hcs/domains/<profile>/rules/` 只加载当前选中的 Profile.
- `.hcs/rules/shared/` 是独立部署的共享规则, 不参与 `AGENTS.md` 渲染, 不出现在生成 manifest 中.
- `.hcs/agents/` 中的 Subagent 共享 Markdown body, 但保留 Codex, Cursor 和 OpenCode 各自的原生 metadata.
...
3. 为新增分支, 解析规则或安全行为补充一个最小可运行测试.
4. 运行 `npm run verify`.
5. 重新注册或修改 CLI 入口时运行 `npm link`.
6. 从一个真实或隔离的外部 `.hcs` 配置目录运行 `hcs generate` 和 `hcs check`.
7. 修改部署逻辑时使用临时 `USERPROFILE` 验证 `hcs setup`, 不触碰真实用户配置.
...
- `npm test` 会先构建, 再使用 `node --test` 运行 `dist/tests/hcs.test.js`.
- 工具工程不包含 `.hcs`, 因此不得在本仓库根目录使用 npm script 包装 `generate`, `check` 或 `setup`.
- Windows sandbox 可能阻止 Node test runner 创建子进程. 发生真实权限错误时在获得权限后复跑, 不修改测试绕过边界.

## 部署安全

- `generate` 只更新调用目录中的 `.hcs/generated/`.
...
- 常规开发验证不得对真实用户目录运行 `setup`.
- 修改部署逻辑后, 使用临时 `USERPROFILE` 验证文件名, 内容和目录边界, 再删除临时目录.
- 不递归删除含有 reparse point 的部署目标.

## 语言与文档

- 常规对话与 Markdown 使用中文文字和英文标点.
...
- 需要持久记录的会话资料放在配套配置目录的 `.agent-sessions/{YYYYMMDD}-{short-kebab-case-session-summary}/`.

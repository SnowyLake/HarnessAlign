...
const MARKER = "<!-- Generated by Harness Config Synchro. Do not edit. -->";
...
export class HcsError extends Error
{
...
function assertContained(root: string, path: string, label: string): void
{
  ...
  {
    throw new HcsError(`${label}: path must stay inside the project root`);
  }
}
...
function reparseError(root: string, path: string, output: boolean): HcsError
{
  return new HcsError(
...
async function readUtf8(root: string, path: string, context = display(root, path)): Promise<string>
{
  ...
  {
    throw new HcsError(`${context}: expected UTF-8 text, got ${errorText(error)}`);
  }
}
...
): Promise<string[]>
{
  ...
  {
    throw new HcsError(`${display(root, directory)}: expected a directory`);
  }
...
async function parseFrontmatter(root: string, path: string): Promise<[Record<string, unknown>, string]>
{
  ...
  if (!match)
  {
    throw new HcsError(`${display(root, path)}: frontmatter must start with a YAML mapping`);
  }
  ...
  {
    throw new HcsError(`${display(root, path)}: invalid YAML frontmatter: ${errorText(error)}`);
  }
  if (!isRecord(metadata))
  {
    throw new HcsError(`${display(root, path)}: frontmatter must be a mapping, got ${typeText(metadata)}`);
  }
  const body = text.slice(match[0].length);
  if (!body.trim())
  {
    throw new HcsError(`${display(root, path)}: Markdown body must be non-empty, got ${valueText(body)}`);
  }
...
function stringArray(value: unknown, path: string, field: string): string[]
{
  ...
  {
    throw new HcsError(`${path}: ${field} must be a non-empty string array, got ${valueText(value)}`);
  }
  if (new Set(value).size !== value.length)
  {
    throw new HcsError(`${path}: ${field} must not contain duplicates, got ${valueText(value)}`);
  }
  return value;
}
...
{
  const path = ".hcs/config.json";
  if (!isRecord(value)) throw new HcsError(`${path}: expected a mapping`);
  for (const field of ["version", "default_profile", "profiles", "harnesses"])
  {
    if (!hasOwn(value, field)) throw new HcsError(`${path}: ${field} is required`);
  }
  if (typeof value.version !== "number" || !Number.isInteger(value.version) || value.version !== 1)
  {
    throw new HcsError(`${path}: version must be integer 1, got ${valueText(value.version)}`);
  }
  if (typeof value.default_profile !== "string" || !value.default_profile)
  {
    throw new HcsError(`${path}: default_profile must be a non-empty string, got ${valueText(value.default_profile)}`);
  }
  const name = hasOwn(value, "name") ? value.name : "AGENTS";
  if (typeof name !== "string" || !name.trim() || /[\r\n]/u.test(name))
  {
    throw new HcsError(`${path}: name must be a non-empty single-line string, got ${valueText(name)}`);
  }
  ...
  {
    throw new HcsError(`${path}: default_profile must be included in profiles, got ${valueText(value.default_profile)}`);
  }
  const unsafeProfile = profiles.find((profile) => profile === "." || profile === ".." || /[\\/]/u.test(profile));
  if (unsafeProfile !== undefined)
  {
    throw new HcsError(`${path}: profiles must contain single directory names, got ${valueText(unsafeProfile)}`);
  }
  const unsupported = rawHarnesses.find((harness) => !isHarness(harness));
  if (unsupported !== undefined)
  {
    throw new HcsError(`${path}: harnesses may only contain codex, cursor, opencode, got ${valueText(unsupported)}`);
  }
...
{
  const path = join(root, ".hcs", "config.json");
  await ensureRegularSource(root, path);
  if (!(await lstatIfExists(path)))
  {
    throw new HcsError(".hcs/config.json: file is required");
  }
  let parsed: unknown;
  try
  {
    parsed = JSON.parse(await readUtf8(root, path, ".hcs/config.json")) as unknown;
  }
  catch (error)
  {
    if (error instanceof HcsError) throw error;
    throw new HcsError(`.hcs/config.json: invalid JSON: ${errorText(error)}`);
  }
...
{
  const hcs = join(root, ".hcs");
  const rulesDirectory = join(hcs, "rules");
  const paths = [
    ...(await markdownFiles(root, rulesDirectory, true, [join(rulesDirectory, "shared")])),
    ...(await markdownFiles(root, join(hcs, "domains", profile, "rules"), true)),
  ...
  for (const sourcePath of paths)
  {
    ...
    {
      throw new HcsError(`${path}: path conflicts with ${existing}; paths may not differ only by case`);
    }
    ...
    if (unknown.length > 0) throw new HcsError(`${path}: unknown rule field ${valueText(firstSorted(unknown))}`);
    if (!hasOwn(metadata, "priority")) throw new HcsError(`${path}: priority is required`);
    const priority = metadata.priority;
    if (typeof priority !== "number" || !Number.isInteger(priority) || priority < 0)
    {
      throw new HcsError(`${path}: priority must be a non-negative integer, got ${valueText(priority)}`);
    }
    let targets: Harness[];
    ...
    else
    {
      ...
      {
        throw new HcsError(`${path}: targets may only contain codex, cursor, opencode, got ${valueText(invalid)}`);
      }
...
function validateString(path: string, field: string, value: unknown): string
{
  ...
  {
    throw new HcsError(`${path}: ${field} must be a non-empty string, got ${valueText(value)}`);
  }
  return value;
}
...
{
  const paths = await markdownFiles(root, join(root, ".hcs", "agents"), false);
  const agents: Agent[] = [];
  ...
  for (const sourcePath of paths)
  {
    ...
    if (unknown.length > 0) throw new HcsError(`${path}: unknown agent field ${valueText(firstSorted(unknown))}`);
    for (const field of AGENT_FIELDS)
    {
      if (!hasOwn(metadata, field)) throw new HcsError(`${path}: ${field} is required`);
    }
    const name = validateString(path, "name", metadata.name);
    if (!AGENT_NAME.test(name)) throw new HcsError(`${path}: name must match ${AGENT_NAME.source}, got ${valueText(name)}`);
    const foldedName = name.toLowerCase();
    if (names.has(foldedName))
    {
      throw new HcsError(`${path}: name must be unique without case sensitivity, got ${valueText(name)}`);
    }
    ...
    {
      throw new HcsError(`${path}: harnesses must be a mapping, got ${typeText(metadata.harnesses)}`);
    }
    const invalidHarness = Object.keys(metadata.harnesses).find((harness) => !isHarness(harness));
    if (invalidHarness !== undefined)
    {
      throw new HcsError(`${path}: harnesses may only contain codex, cursor, opencode, got ${valueText(invalidHarness)}`);
    }
    ...
    for (const [harnessName, rawValues] of Object.entries(metadata.harnesses))
    {
      ...
      {
        throw new HcsError(`${path}: ${harness} metadata must be a mapping, got ${typeText(rawValues)}`);
      }
      ...
      {
        throw new HcsError(`${path}: unknown ${harness} field ${valueText(firstSorted(invalidField))}`);
      }
      for (const [field, value] of Object.entries(rawValues))
      {
        ...
        {
          if (typeof value !== "boolean") throw new HcsError(`${path}: ${qualified} must be a boolean, got ${typeText(value)}`);
        }
        else if (harness === "opencode" && field === "permission")
        {
          if (!isRecord(value)) throw new HcsError(`${path}: ${qualified} must be a mapping, got ${typeText(value)}`);
          for (const [permission, mode] of Object.entries(value))
          {
            if (typeof mode !== "string" || !["allow", "ask", "deny"].includes(mode))
            {
              throw new HcsError(`${path}: ${qualified}.${permission} must be allow, ask, or deny, got ${valueText(mode)}`);
            }
          }
        }
        else if (typeof value !== "string" || !value)
        {
          throw new HcsError(`${path}: ${qualified} must be a non-empty string, got ${valueText(value)}`);
        }
      }
    ...
    {
      throw new HcsError(`${path}: at least one configured harness block is required`);
    }
...
export async function buildOutputs(rootPath: string, profile?: string): Promise<OutputMap>
{
  ...
  {
    throw new HcsError(`.hcs/config.json: profile must be configured, got ${valueText(selected)}`);
  }
...
// generated 根目录本身也必须拒绝链接. 仅检查最终文件不足以阻止 `.hcs/generated` 被 junction 引到项目外.
async function outputRoot(root: string): Promise<string>
{
  const path = join(root, ".hcs", "generated");
  const stats = await lstatIfExists(path);
  if (stats?.isSymbolicLink()) throw new HcsError(".hcs/generated: symbolic link output directories are not allowed");
  if (stats && !stats.isDirectory()) throw new HcsError(".hcs/generated: expected a directory");
  return path;
}
...
export function safeOutputRelative(value: unknown): string
{
  ...
  {
    throw new HcsError(`.hcs/generated/.manifest.json: invalid managed path ${valueText(value)}`);
  }
...
async function destination(root: string, outputRelative: string): Promise<string>
{
  ...
  assertContained(generated, path, ".hcs/generated");
  ...
  for (const part of parts.slice(0, -1))
  {
    ...
    if (stats && !stats.isDirectory()) throw new HcsError(`${display(root, current)}: expected an output directory`);
  }
  const stats = await lstatIfExists(path);
  if (stats?.isSymbolicLink()) throw reparseError(root, path, true);
  if (stats && !stats.isFile()) throw new HcsError(`${display(root, path)}: managed output must be a file`);
  return path;
}
...
async function loadManifest(root: string): Promise<string[]>
{
  ...
  const stats = await lstatIfExists(path);
  if (!stats) return [];
  if (stats.isSymbolicLink()) throw new HcsError(".hcs/generated/.manifest.json: symbolic link outputs are not allowed");
  if (!stats.isFile()) throw new HcsError(".hcs/generated/.manifest.json: managed output must be a file");
  let manifest: unknown;
  try
  {
    manifest = JSON.parse(await readUtf8(root, path, ".hcs/generated/.manifest.json")) as unknown;
  }
  catch (error)
  {
    if (error instanceof HcsError) throw error;
    throw new HcsError(`.hcs/generated/.manifest.json: invalid manifest: ${errorText(error)}`);
  }
  if (!isRecord(manifest) || typeof manifest.version !== "number" || !Number.isInteger(manifest.version) || manifest.version !== 1)
  {
    throw new HcsError(".hcs/generated/.manifest.json: version must be integer 1");
  }
  if (!Array.isArray(manifest.files) || !manifest.files.every((file) => typeof file === "string"))
  {
    throw new HcsError(".hcs/generated/.manifest.json: files must be a string array");
  }
  if (new Set(manifest.files).size !== manifest.files.length)
  {
    throw new HcsError(".hcs/generated/.manifest.json: files must not contain duplicates");
  }
  if (manifest.files.includes(".manifest.json"))
  {
    throw new HcsError(".hcs/generated/.manifest.json: files must not manage the manifest itself");
  }
...
): Promise<void>
{
  ...
  const temporary = join(parent, `.hcs-${randomUUID()}.tmp`);
  try
  {
...
function assertContainedWithin(root: string, path: string, label: string): string
{
  ...
  {
    throw new HcsError(`${label}: path must stay inside its allowed root`);
  }
  return pathFull;
}

function deploymentReparseError(label: string, path: string): HcsError
{
  return new HcsError(`${label}: reparse points are not allowed: ${path}`);
}
...
async function assertNoReparseComponents(root: string, path: string, label: string): Promise<string>
{
  ...
  if (!rootStats || !rootStats.isDirectory()) throw new HcsError(`${label}: allowed root must be an existing directory: ${rootFull}`);
...
{
  const stats = await lstatIfExists(path);
  if (!stats) throw new HcsError(`${label}: directory does not exist: ${path}`);
  if (stats.isSymbolicLink()) throw deploymentReparseError(label, path);
  if (!stats.isDirectory()) throw new HcsError(`${label}: expected a directory: ${path}`);
}

async function assertRegularFile(path: string, label: string): Promise<void>
{
  const stats = await lstatIfExists(path);
  if (!stats) throw new HcsError(`${label}: file does not exist: ${path}`);
  if (stats.isSymbolicLink()) throw deploymentReparseError(label, path);
  if (!stats.isFile()) throw new HcsError(`${label}: expected a file: ${path}`);
}
...
{
  const stats = await lstatIfExists(path);
  if (!stats) throw new HcsError(`${label}: path does not exist: ${path}`);
  if (stats.isSymbolicLink()) throw deploymentReparseError(label, path);
...
async function removeDeploymentDirectory(userProfile: string, path: string, label: string): Promise<void>
{
  ...
  if (!stats) return;
  if (!stats.isDirectory()) throw new HcsError(`${label}: expected a directory: ${target}`);
...
export async function setup(rootPath: string, profile?: string, userProfile = process.env.USERPROFILE): Promise<void>
{
  ...
  {
    throw new HcsError(`USERPROFILE must be an absolute path, got ${valueText(userProfile)}`);
  }
  ...
  for (const target of targets)
  {
    ...
    if (agentsStats)
    {
      if (!agentsStats.isDirectory()) throw new HcsError(`${target.harness} target agents: expected a directory: ${targetAgents}`);
  ...
  const sourceSharedRules = await assertNoReparseComponents(root, join(root, ".hcs", "rules", "shared"), "shared rules source");
  ...
  if (sharedStats)
  {
    if (!sharedStats.isDirectory()) throw new HcsError(`shared rules target: expected a directory: ${targetSharedRules}`);
...
function usage(error?: string): number
{
  ...
  process.stderr.write("usage: hcs <generate|check|setup> [--profile <profile>]\n");
  return 2;
}
...
export async function main(argv: string[], root = process.cwd()): Promise<number>
{
  ...
  {
    process.stdout.write("usage: hcs <generate|check|setup> [--profile <profile>]\n");
    return 0;
  }

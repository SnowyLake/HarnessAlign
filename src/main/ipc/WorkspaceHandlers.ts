...
{
    ipcMain.handle(IPC_CHANNELS.workspaceLoad, (event) => runIpc(event, async () =>
    {
    ...
    ipcMain.handle(IPC_CHANNELS.workspaceRenameSource, (event, from: unknown, to: unknown) => runIpc(event, async () =>
    {
        await workspaceService.renameSource(z.string().parse(from), z.string().parse(to));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceAddLayer, (event, name: unknown) => runIpc(event, async () =>
    {
        return workspaceService.addLayer(z.string().parse(name));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceRemoveLayer, (event, name: unknown) => runIpc(event, async () =>
    {
        return workspaceService.removeLayer(z.string().parse(name));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceRenameLayer, (event, from: unknown, to: unknown) => runIpc(event, async () =>
    {
        return workspaceService.renameLayer(z.string().parse(from), z.string().parse(to));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceAddLayerOption, (event, layer: unknown, option: unknown) => runIpc(event, async () =>
    {
        await workspaceService.addLayerOption(z.string().parse(layer), z.string().parse(option));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceRemoveLayerOption, (event, layer: unknown, option: unknown) => runIpc(event, async () =>
    {
        await workspaceService.removeLayerOption(z.string().parse(layer), z.string().parse(option));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceRenameLayerOption, (event, layer: unknown, from: unknown, to: unknown) => runIpc(event, async () =>
    {
        return workspaceService.renameLayerOption(z.string().parse(layer), z.string().parse(from), z.string().parse(to));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceAddHarness, (event, harness: unknown) => runIpc(event, async () =>
    {
        return workspaceService.addHarness(HARNESS_SCHEMA.parse(harness));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceRemoveHarness, (event, name: unknown) => runIpc(event, async () =>
    {
    ...
    ipcMain.handle(IPC_CHANNELS.workspaceRemoveSkillSource, (event, owner: unknown, name: unknown) => runIpc(event, async () =>
    {
        return workspaceService.removeSkillSource(z.string().parse(owner), z.string().parse(name));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceDiscoverSkills, (event) => runIpc(event, async () =>
    {
    ...
    ipcMain.handle(IPC_CHANNELS.workspaceCheckSkillUpdates, (event) => runIpc(event, async () =>
    {
    ...
    ipcMain.handle(IPC_CHANNELS.workspaceImportUserSkills, (event, ids: unknown, overwrite: unknown) => runIpc(event, async () =>
    {
        return workspaceService.importUserSkills(SKILL_IDS_SCHEMA.parse(ids), z.boolean().parse(overwrite));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceRemoveSkill, (event, id: unknown) => runIpc(event, async () =>
    {
        await workspaceService.removeSkill(z.string().parse(id));
    }));

    ipcMain.handle(IPC_CHANNELS.workspaceGenerate, (event, selection?: unknown) => runIpc(event, async () =>
    {
